...
// ============================================================
// GOOGLE GEMINI AI SERVICE — PRIMARY PROVIDER
// Replaces Grok as the main AI engine.
//
// Models (override in .env):
//   GEMINI_PRIMARY_MODEL → default: gemini-2.0-flash   (free, fast, primary)
//   GEMINI_PRO_MODEL     → default: gemini-1.5-pro     (background tasks)
//   GEMINI_FLASH_MODEL   → default: gemini-1.5-flash   (last-resort fallback)
//
...
export const PRIMARY_MODEL = process.env.GEMINI_PRIMARY_MODEL || 'gemini-2.0-flash';
export const PRO_MODEL     = process.env.GEMINI_PRO_MODEL     || 'gemini-1.5-pro';
export const FLASH_MODEL   = process.env.GEMINI_FLASH_MODEL   || 'gemini-1.5-flash';

// ── Convert our message format to Gemini's format ────────────
// Our app: { role: 'user'|'assistant', content }
...
// ──────────────────────────────────────────
// CORE: Non-streaming Gemini call
// ──────────────────────────────────────────
export const callGemini = async ({
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
      ...
      if (status === 401 || status === 403) throw new Error('GEMINI_AUTH_ERROR');
      if (status === 400) throw new Error('GEMINI_BAD_REQUEST');
      if (attempt === 3) throw new Error(`GEMINI_UNAVAILABLE: ${err.message}`);
...
// ──────────────────────────────────────────
// CORE: Streaming Gemini call
// Same callback interface as the old streamGrok.
// ──────────────────────────────────────────
export const streamGemini = async ({
  ...
  try {
    ...
    onComplete?.(fullContent, {
      tokens_in:  usage.promptTokenCount     || 0,
      tokens_out: usage.candidatesTokenCount || Math.ceil(fullContent.length / 4),
      model_used: modelName
    });
...
Example: ["Question 1?", "Question 2?"]`;
  ...
  try {
    ...
    if (valid.length >= 3) {
      console.log('[Gemini] generateOnboardingQuestions: AI questions generated successfully');
      return { questions: valid, source: 'ai' };
    }

    console.warn('[Gemini] generateOnboardingQuestions: AI returned too few valid questions, using fallback. Raw:', content?.slice(0, 200));
    return { questions: FALLBACK, source: 'fallback' };
  } catch (err) {
    console.error('[Gemini] generateOnboardingQuestions FAILED:', err.message);
    return { questions: FALLBACK, source: 'fallback' };
  }
};
...
- Sound like a human who actually read their post, not an AI that scanned keywords`;
  ...
  try {
    const { content, tokens_in, tokens_out } = await callGemini({
      messages: [{ role: 'user', content: prompt }],
      temperature: 0.82,
      maxTokens: 350
    });
...
// Returns { systemPrompt, messages } — provider-agnostic builder
// ──────────────────────────────────────────
...
// Always uses Pro model for quality analysis.
// ──────────────────────────────────────────
...
export const summarizePerformancePatterns = async (user, sentOpps, feedbackData) => {
  ...
  const systemPrompt = `You are a battle-tested sales mentor analyzing outreach performance data.
Be specific and data-driven. Give concise, actionable insights.`;

  const userPrompt = `Analyze this founder's outreach data and write a 2-sentence insight summary.
This will be used to automatically improve their future AI-generated messages.
...
Example of a good summary: "Best results on Reddit (40% positive). Short messages under 80 words with empathetic openers outperform longer direct pitches 3:1."
Return ONLY the 2-sentence summary. No JSON. No preamble.`;
  ...
  try {
    const { content } = await callGemini({
      ...
      temperature: 0.3,
      maxTokens:   200,
      modelName:   PRO_MODEL   // Pro for background analysis
    });
    return parseTextResponse(content, null);
  } catch (err) {
    console.error('[Gemini] summarizePerformancePatterns failed:', err.message);
    return null;
  }
};
