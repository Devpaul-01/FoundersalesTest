...
function LogEntry({ log }) {
  ...
  return (
    <div className="border-b border-white/5 last:border-0">
      ...
      {/* Expanded data */}
      {expanded && hasData && (
        <div className="mx-3 mb-2 rounded-lg bg-black/50 border border-white/5 overflow-auto max-h-64">
          {log.message && (
  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={openPanel}
          className="fixed bottom-24 right-4 z-50 flex items-center gap-1.5 px-3 py-2 rounded-xl bg-gray-900 border border-white/10 shadow-xl text-xs font-mono text-gray-300 hover:border-white/20 transition-all"
        >
      ...
      {/* Panel */}
      {isOpen && (
        <div className="fixed bottom-0 right-0 z-50 w-full sm:w-[520px] h-[60vh] sm:h-[70vh] sm:bottom-4 sm:right-4 sm:rounded-2xl bg-[#0d1117] border border-white/10 shadow-2xl flex flex-col overflow-hidden">
