...
export default function AppLayout() {
  ...
  // Hide the bottom nav on chat and practice pages — it covers the input box
  const hideBottomNav =
  ...
  return (
    <div className="flex h-screen w-full bg-surface-bg overflow-hidden">
      ...
      {/* Mobile Bottom Nav — hidden on chat/practice to avoid covering the input */}
      {!hideBottomNav && (
  ...
  return (
    <div
      className={[
        ...
        // Only add bottom padding for BottomNav when it's actually visible
        !noBottomNav ? 'pb-24 lg:pb-6' : 'lg:pb-6',
        className
      ].filter(Boolean).join(' ')}
    >
      {children}
