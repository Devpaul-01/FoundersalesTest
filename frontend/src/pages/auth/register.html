...
//   2. Backend creates auth user (NOT auto-confirmed)
//   3. Supabase sends verification email
//   4. Page shows "Check your email" state
//   5. User clicks link → /auth/callback → onboarding
//
// Google OAuth:
//   1. User clicks "Continue with Google"
//   2. Redirect to Google via Supabase OAuth
//   3. Callback handled by /auth/callback page
// ============================================================
...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.needsVerification) {
        ...
        // Dev/auto-confirm path (shouldn't happen in production)
  ...
  const handleResend = async () => {
    setResending(true)
    try {
      await api.post('/auth/resend-verification', { email: sentEmail })
  ...
  const handleGoogleSignup = async () => {
    try {
      ...
      toast.error('Google signup unavailable')
    }
  }
      ...
      <div className="min-h-screen bg-surface-bg grid-bg flex items-center justify-center p-4">
        <div className="w-full max-w-sm relative">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold font-display text-text-primary">Check your email</h1>
            <p className="text-sm text-text-muted mt-2">
              We sent a verification link to
            </p>
            <p className="text-sm font-medium text-text-primary mt-1">{sentEmail}</p>
          </div>

          <div className="bg-surface-card border border-surface-border rounded-2xl p-6 shadow-modal">
            <p className="text-sm text-text-secondary mb-5 text-center leading-relaxed">
              Click the link in the email to verify your account, then you'll be taken to set up your profile.
            </p>

            <div className="space-y-3">
              <Button variant="secondary" fullWidth onClick={handleResend} loading={resending}>
                Resend verification email
              </Button>
              <button
                onClick={() => { setVerificationSent(false); setSentEmail('') }}
    ...
    <div className="min-h-screen bg-surface-bg grid-bg flex items-center justify-center p-4">
      ...
      <div className="w-full max-w-sm relative">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-text-muted mt-1">Start closing deals today</p>
        </div>

        <div className="bg-surface-card border border-surface-border rounded-2xl p-6 shadow-modal">
          <h2 className="text-base font-semibold text-text-primary mb-5">Create your account</h2>
